...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if ($id) {
        try {
            ...
            $checkStmt = $pdo->prepare("SELECT student_name FROM students_colors WHERE student_id = :student_id");
            $checkStmt->execute(['student_id' => $id]);
            $student = $checkStmt->fetch(PDO::FETCH_ASSOC);

            if ($student) {
                $stmt = $pdo->prepare("DELETE FROM students_colors WHERE student_id = :student_id");
                $stmt->execute(['student_id' => $id]);
                
                if ($stmt->rowCount() > 0) {
                    $response = ['status' => 'success', 'message' => 'Student\'s color deleted successfully', 'deleted_student' => $student['student_name']];
                } else {
                    $response = ['status' => 'error', 'message' => 'Failed to delete student\'s color'];
                }
            } else {
